...
def config_dir(tmpdir):
    ...
    test_dir.join('config.cfg').write_text('')
    test_dir.join('extra.ini').write_text('')
    test_dir.join('module.cfg').write_text('')
    test_dir.join('README').write_text('')
...
def test_get_pid_filename_default(configfactory):
    ...
    assert result == os.path.join(pid_dir, 'sopel.pid')
...
def test_get_pid_filename_named(configfactory):
    ...
    result = get_pid_filename(settings, pid_dir)
    assert result == os.path.join(pid_dir, 'sopel-test.pid')
...
def test_get_pid_filename_ext_not_cfg(configfactory):
    ...
    result = get_pid_filename(settings, pid_dir)
    assert result == os.path.join(pid_dir, 'sopel-test.ini.pid')
...
def test_get_running_pid(tmpdir):
    ...
    result = get_running_pid(str(pid_file))
    assert result == 7814
