...
from sopel.config.types import ChoiceAttribute, StaticSection, BooleanAttribute
...
class HelpSection(StaticSection):
    ...
    show_server_host = BooleanAttribute('show_server_host',
                                              default=True)
...
def post_to_termbin(msg):
    ...
    # find/replace just in case the site tries to be sneaky and save on SSL overhead,
    # though it will probably send us an HTTPS link without any tricks.
    return response.strip(b'\x00\n').replace(b'http://', b'https://', 1).decode('utf-8')
...
def create_list(bot, msg):
    ...
    try:
        result = PASTEBIN_PROVIDERS[bot.config.help.output](msg.encode('utf-8')).decode('utf-8')
    except PostingException:
