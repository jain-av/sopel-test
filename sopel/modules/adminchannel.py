...
from sopel import formatting, plugin, tools
from sopel.permissions import OP, HALFOP
...
@plugin.require_chanmsg
@plugin.require_privilege(OP, ERROR_MESSAGE_NO_PRIV, reply=True)
@plugin.require_bot_privilege(OP, ERROR_MESSAGE_NOT_OP, reply=True)
@plugin.command('op')
def op(bot, trigger):
    """
    Command to op users in a room. If no nick is given,
...
@plugin.require_chanmsg
@plugin.require_privilege(OP, ERROR_MESSAGE_NO_PRIV, reply=True)
@plugin.require_bot_privilege(HALFOP, ERROR_MESSAGE_NOT_OP, reply=True)
@plugin.command('voice')
def voice(bot, trigger):
    """
    Command to voice users in a room. If no nick is given,
...
def topic(bot, trigger):
    ...
    """
    channel = trigger.sender.lower()
    chan = bot.channels[trigger.sender]
    mode_t = 't' in chan.modes
    user_op = bot.channels[trigger.sender].privileges[trigger.nick] >= HALFOP

    if mode_t and not bot.has_channel_privilege(trigger.sender, HALFOP):
        bot.reply(ERROR_MESSAGE_NOT_OP)
        return
    if mode_t and not user_op:
        bot.reply(ERROR_MESSAGE_NO_PRIV)
        return
    if not trigger.group(2):
        return
    ...
    topiclen = bot.network.isupport.get('TOPICLEN')
    if topiclen is not None:
        my_len = len(topic.encode('utf-8'))
        if my_len > topiclen:
            bot.reply(
...
@plugin.require_chanmsg
@plugin.require_privilege(OP, ERROR_MESSAGE_NO_PRIV)
@plugin.command('tmask')
def set_mask(bot, trigger):
    """Set the topic mask to use for the current channel
