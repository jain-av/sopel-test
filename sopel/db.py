...
class SopelDB(object):
    ...
    def get_nick_id(self, nick, create=True):
        ...
        slug = nick.lower()
        try:
            nickname = session.execute(select(Nicknames).where(Nicknames.slug == slug)).scalar_one_or_none()

            if nickname is None:
                # see if it needs case-mapping migration
                nickname = session.execute(select(Nicknames).where(Nicknames.slug == Identifier._lower_swapped(nick))).scalar_one_or_none()
    ...
    def alias_nick(self, nick, alias):
        ...
        session = self.ssession()
        try:
            result = session.execute(select(Nicknames).where(Nicknames.slug == alias.lower()).where(Nicknames.canonical == alias)).scalar_one_or_none()
            if result:
    ...
    def set_nick_value(self, nick, key, value):
        ...
        session = self.ssession()
        try:
            result = session.execute(select(NickValues).where(NickValues.nick_id == nick_id).where(NickValues.key == key)).scalar_one_or_none()
    ...
    def get_nick_value(self, nick, key, default=None):
        ...
        session = self.ssession()
        try:
            result = session.execute(select(NickValues.value).join(Nicknames, Nicknames.nick_id == NickValues.nick_id).where(Nicknames.slug == nick.lower()).where(NickValues.key == key)).scalar_one_or_none()
    ...
    def unalias_nick(self, alias):
        ...
        session = self.ssession()
        try:
            count = session.execute(select(func.count()).select_from(Nicknames).where(Nicknames.nick_id == nick_id)).scalar()
            if count <= 1:
                raise ValueError('Given alias is the only entry in its group.')
            session.execute(delete(Nicknames).where(Nicknames.slug == alias.lower()))
            session.commit()
        except SQLAlchemyError:
            session.rollback()
            raise
...
from sqlalchemy import Column, create_engine, ForeignKey, Integer, String, select, update, delete
from sqlalchemy.engine.url import URL
from sqlalchemy.exc import OperationalError, SQLAlchemyError
...
class SopelDB(object):
    ...
    def get_nick_id(self, nick, create=True):
        ...
        slug = nick.lower()
        try:
            nickname = session.execute(select(Nicknames).filter(Nicknames.slug == slug)).scalar_one_or_none()

            if nickname is None:
                # see if it needs case-mapping migration
                nickname = session.execute(select(Nicknames).filter(Nicknames.slug == Identifier._lower_swapped(nick))).scalar_one_or_none()
                if nickname is not None:
                    # it does!
                    nickname.slug = slug
                    session.commit()
    ...
    def get_nick_value(self, nick, key, default=None):
        ...
        session = self.ssession()
        try:
            statement = select(NickValues.value) \
    ...
    def unalias_nick(self, alias):
        ...
        try:
            ...
            if count <= 1:
                raise ValueError('Given alias is the only entry in its group.')
            delete_stmt = delete(Nicknames).where(Nicknames.slug == alias.lower())
    ...
    def delete_nick_group(self, nick):
        ...
        session = self.ssession()
        try:
            delete_nicknames = delete(Nicknames).where(Nicknames.nick_id == nick_id)
            session.execute(delete_nicknames)
            delete_nickvalues = delete(NickValues).where(NickValues.nick_id == nick_id)
            session.execute(delete_nickvalues)
            session.commit()
        except SQLAlchemyError:
            session.rollback()
            raise
    ...
    def merge_nick_groups(self, first_nick, second_nick):
        ...
        session = self.ssession()
        try:
            result = session.execute(select(PluginValues.value).where(PluginValues.plugin == plugin, PluginValues.key == key)).scalar_one_or_none()
