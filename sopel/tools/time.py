...
def get_nick_timezone(db, nick):
    ...
    """
    try:
        # Assuming db.get_nick_value returns a sqlalchemy.ScalarResult
        result = db.get_nick_value(nick, 'timezone')
        return validate_timezone(result) if result else None
    except ValueError:
        return None
...
def get_channel_timezone(db, channel):
    ...
    """
    try:
        # Assuming db.get_channel_value returns a sqlalchemy.ScalarResult
        result = db.get_channel_value(channel, 'timezone')
        return validate_timezone(result) if result else None
    except ValueError:
        return None
...
def get_timezone(db=None, config=None, zone=None, nick=None, channel=None):
    ...
    if zone:
        tz = _check(zone)
        if not tz:
            # Assuming db.get_nick_or_channel_value returns a sqlalchemy.ScalarResult
            result = db.get_nick_or_channel_value(zone, 'timezone')
            tz = _check(result) if result else None
    if not tz and nick:
        result = db.get_nick_value(nick, 'timezone')
        tz = _check(result) if result else None
...
def format_time(db=None, config=None, zone=None, nick=None, channel=None,
    ...
    if db:
        if nick:
            result = db.get_nick_value(nick, 'time_format')
            tformat = result
        if not tformat and channel:
            result = db.get_channel_value(channel, 'time_format')
            tformat = result
...
def seconds_to_human(secs, granularity=2):
    ...
    if secs == 0:
        ...
        time_units = get_time_unit(*seconds_to_split(secs))
        result = ", ".join([
            f"{value} {unit}"
